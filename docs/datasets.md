...
post-filters results to the exact owner, and proxies authorized private media
bytes to the browser. The token, upstream provider URL, and local cache path
remain server-side.
...
Scripts can browse the same data with `CtrlPiClient.list_datasets()`,
`list_dataset_episodes()`, and `get_dataset_episode()`. The returned
`video_url` names the same-origin, revision-pinned byte-range REST endpoint;
the synchronous JSON client deliberately does not buffer video bytes. See
