    ...
    <section className="rounded-xl border border-slate-200 bg-white shadow-panel">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        <dl className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Detail label="Provider / endpoint"><span className={state?.endpoint_healthy ? "text-emerald-700" : "text-slate-500"}>{state?.endpoint_healthy ? (state.session_status === "idle" ? "Provider ready · identity verified at deploy" : "Healthy · identity verified") : deployment.endpoint_url ? "Assigned · awaiting readiness" : "Not available"}</span><br /><span className="font-mono text-[10px] font-normal text-slate-400" title={deployment.endpoint_url ?? undefined}>{deployment.provider_app_id ?? deployment.endpoint_id}</span></Detail>
